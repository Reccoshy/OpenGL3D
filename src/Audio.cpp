...
namespace Audio {
	...
	*/
	...
	@retval false 初期化失敗.
	*/
	bool Initialize(const char* acfPath, const char* acbPath, const char* awbPath,
	...
	*/
	...
	@param playerId	音声を再開させるプレイヤーのID.
	*/
	void Resume(int playerId)
	{
		criAtomExPlayer_Pause(playerList[playerId], CRI_FALSE);
	}

	/*
	音声を一時停止させる.

	@param playerId	再生を一時停止させるプレイヤーのID.
	*/
	void Pause(int playerId)
	{
		criAtomExPlayer_Pause(playerList[playerId], CRI_TRUE);
	}
