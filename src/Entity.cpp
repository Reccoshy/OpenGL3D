...
namespace Entity {
	...
	@param entity	転送データを保持する
	@param ubo		転送先バッファへのポインタ.
	@param matMVP	転送する座標変換行列(ビューとプロジェクション).
	*/
	...
	*/
	Entity* Buffer::AddEntity(int groupId, const glm::vec3& position, const Mesh::MeshPtr& mesh,
		...
		Entity::UpdateFuncType func, bool isInitialWrite)
	{
		...
		if (groupId < 0 || groupId > maxGroupId) {
			std::cerr << "ERROR in Entity::Buffer::AddEntity: 範囲外のグループID(" << groupId <<
				")が渡されました.\nグループIDは0～" << maxGroupId << "出なければいけません." <<
				std::endl;
			return nullptr;
		}
		...
		entity->initialWriteOnly = isInitialWrite;
